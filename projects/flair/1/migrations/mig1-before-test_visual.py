from flair.visual import *		from flair.visual import *
from flair.data import Sentence		from flair.data import Sentence
from flair.embeddings import CharLMEmbeddings, StackedEmbeddings		from flair.embeddings import CharLMEmbeddings, StackedEmbeddings
import unittest		
...
class Test(unittest.TestCase):		
    ...
    def test_char_contexts(self):		@pytest.mark.skip(reason='Skipping test by default due to long execution time.')
def test_benchmark():
    import time
...
class TestHighlighter(unittest.TestCase):		    trans_ = tSNE()
    def test(self):		    reduced = trans_.fit(X)


        i = numpy.random.choice(2048)		    l = len(sentences[0])
        ...
        embeddings = CharLMEmbeddings('news-forward')		    show(reduced[:l], contexts[:l])
